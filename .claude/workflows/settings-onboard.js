...
5. Ensure .claude/hooks/packs.toml exists — capt-hook loads ZERO packs without it. If missing, copy the right flavor from cc-skills: plugins/repo-bootstrap/skills/repo-bootstrap/templates/{python|go|base}/claude/hooks/packs.toml (fetch via gh api repos/yasyf/cc-skills/contents/... if you're in a scratchpad clone; base flavor for swift/other). Also ensure .claude/jj-config.toml exists (base template, author from git log -1 --format='%an|%ae' of THIS repo); for python repos also .claude/ty-quiet.toml (python template).
6. Install cc-guides if needed (brew list cc-guides || brew install yasyf/tap/cc-guides) and run cc-guides render at repo root. First render over a pre-existing hand-written settings.json will refuse with "refusing to overwrite a handwritten file" — that is the expected one-time onboarding guard; re-run with --force ONLY after step 1 saved the old file.
7. RESIDUE GATE: diff <(jq -S . /tmp/old-settings-${r.repo}.json) <(jq -S . .claude/settings.json). Every only-in-old line must be either (a) explicitly covered by the overlay instruction above, (b) a stale "hooks" block or a stray cc-context@skills entry (deliberately dropped), or (c) a value identical to what base provides. ANYTHING ELSE → move it into the overlay and re-render, or if it semantically conflicts with base, STOP and check back. Rendered file must have enabledPlugins["captain-hook@captain-hook"] == true and extraKnownMarketplaces.captain-hook.
8. Verify cc-guides check --diff green. ONE commit that includes EVERY artifact this render refreshed — layout.toml + overlay + rendered .claude/settings.json + lock + any re-rendered AGENTS.md/CLAUDE.md (render is unscoped and advances the shared cc-guides pin, so those artifacts move even when settings.json is the target) + any added packs.toml/jj-config.toml/ty-quiet.toml — message "chore: render .claude/settings.json from cc-guides fragments". Committing selectively (settings.json + lock but not the re-rendered AGENTS.md/CLAUDE.md) reds the guides.yml check CI. HOW you commit depends on the clone: for a LOCAL shared clone at /Users/yasyf/Code/${r.repo}, commit exactly those named files fileset-scoped (never \`ccx vcs ship\` — its unscoped commit would sweep a concurrent session's files), push, then watch guides.yml; on push reject fetch + rebase onto main@origin and re-push. Only in a fresh scratchpad clone (nothing else touching the tree) is \`ccx vcs ship -m "<msg>"\` fine — it commits + pushes + watches in one. Watch: gh run list -R yasyf/${r.repo} --workflow guides.yml -L 3 then gh run watch <id> --exit-status.
