...
8. Verify cc-guides check --diff green. ONE commit: layout.toml + overlay + settings.json + lock + any added packs.toml/jj-config.toml/ty-quiet.toml, message "chore: render .claude/settings.json from cc-guides fragments". Push to main; watch the guides.yml run on your commit: gh run list -R yasyf/${r.repo} --workflow guides.yml -L 3 then gh run watch <id> --exit-status (or ccx vcs ship if working in the local clone).
