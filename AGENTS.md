...
12. **Commit, push, watch CI** → `ccx vcs ship -m "<msg>"` (jj-aware commit + push + a watch over every workflow run on the pushed commit, in one call — per-run `workflow · conclusion · duration · url` report, plus failing jobs and a `--budget`-capped log excerpt when a run goes red)
