---
name: writing-docs
description: Use when writing or revising project documentation of any kind, including a README, docs site, tutorial, quickstart, how-to guide, reference page, API doc, cheat sheet, examples catalog, limitations or FAQ page, or changelog. Applies the Diataxis framework (one mode per page), a technical-builder voice (first-person, confident, hands-on), runnable code-sample rules, quickstart design, the canonical README skeleton (banner-in-H1, opener register, one live get-started path with a real-run demo and install-to-run coherence, agent block), standalone-README design, cheat-sheet and catalog page formats, accessibility, a delegated-prose rule (subagents read this skill, never a paraphrase), and a required slop-cop prose-lint pass before you finish. Triggers on "write/rewrite/improve the docs", "write a README", "write a tutorial", "document this", "getting-started page", "write a cheat sheet", "build an examples catalog", or any natural-language documentation task.
---
...
Writing always runs on fable: never delegate prose to a down-routed subagent — inherit the session model or pass `model: fable`. Delegation hands over the model, never the rules: a subagent or workflow prompt that writes docs directs the agent to read this skill and its references (the installed plugin copy or the cc-skills source), and never restates them. A paraphrase drifts — one orchestrator compressed this skill to "technical-builder voice, no hype adjectives", then instructed a stacked install section and a "How it works" internals tour, and the writing agent obeyed the prompt it was given. The delegating prompt does not outrank the skill: it cannot add a section the skeleton drops or waive the single-install rule, and a delegated writer that receives such an instruction surfaces the conflict instead of complying.
...
- Open on substance. Cut self-referential intros such as "This page explains" or "What to learn", structure pre-announcements such as "three properties follow", and pre-emptive admonishment. State behavior as fact, and keep internals, packaging, and history out of task and reference pages — and out of the README. Cut "simply", "just", and "easy" from procedure steps.
...
- Show real, verified output. Normalize variable values — timestamps, addresses, session IDs, ports, generated URLs — to visible placeholders such as `http://127.0.0.1:<port>`; never fake them, and never leave a concrete-looking value the reader might try to match.
...
Keep example code in real source files that are under test, and embed it into the page instead of retyping. One source means the rendered page and the tested code cannot diverge. Make the examples run in CI so a stale example fails the build. If the page shows output, generate it by running the code. Demo media obeys the same law: a screenshot or animated demo shows a real run of the exact command it sits under, and the generator that produced it is committed alongside (`docs/scripts/demo.sh` or a cli-demo tape).

State the run convention once and use it verbatim everywhere. For a tool distributed through uvx or pipx, every command is the full ephemeral form such as `uvx <tool> ...`, never the bare binary, except where a config file must hold it. Leading with the ephemeral invocation is the uv and Ruff house style. For such a tool the ephemeral invocation *is* the install section: a single line such as `uvx <tool> --help` or the first real command. Drop the ceremony around it — no "no install needed" preamble, no gloss on what the runner does ("fetches it into a throwaway environment and runs it"), and no "to add it to a project instead, `uv add <tool>`" alternative unless the tool is genuinely consumed as a library. A reader running `uvx` already knows what it does. The same brevity governs every install, not only uvx ones: name ONE canonical method and stop. Cut the "or build it from source with `go install`" alternative, the "to add it to a project: `uv add`" library line stacked under a CLI, and any second redundant way to fetch the same binary. A genuinely distinct path — a plugin that auto-installs the binary, a real optional extra such as a `[mlx]` group — earns one concise line; a redundant alternative does not.

The one canonical method is a live one, stated from the reader's now — never the maintainer's release calendar. No "goes live with the first tagged release", no "until then", no "coming soon": a README is read after you ship, so timeline narration is stale on arrival. If the canonical path isn't live yet, cutting the release is part of shipping the README. Pre-tap, `go install <module>@latest` can be the one canonical path — it works from any pushed commit — but a from-clone build (`git clone`, `task build`, `./bin/<tool>`) is contributor content for AGENTS.md or a Development section, never the get-started path. The install is also a contract with everything below it: every later command invokes the exact artifact the install produced, same name and same form — `brew install x` means later commands run `x`, never `./bin/x` — with no re-acquisition and no prerequisite (`task`, `bun`) the section doesn't name.
...
A quickstart is a Diataxis tutorial with one language, one use case, and zero branches. State the destination and a time budget up front, as in "Write your first hook in under five minutes". End in one tangible, verifiable outcome the reader can see. Aim for first success under five minutes and at most around ten steps. Give complete copy-paste snippets that say exactly where the code goes, surface every needed value inline, and show the expected output so the reader self-verifies. Defer concepts; give the most minimal inline reason and link to depth. Test the quickstart on a clean machine each release. The example itself sells the tool: pick a first-use case a newcomer recognizes, never an in-joke that only parses if you know the project's own development story. When the repo commits a runnable example, drive the quickstart with that file by path instead of retyping a smaller copy inline; when the only committed example is too big or too inside-baseball for a first run, commit a quickstart-sized one rather than hand-rolling it in the page.

## README anatomy

The README is a front door, not the house: it converts a visitor in one screenful, demonstrates the result before explaining anything, and funnels every deeper question out. The order: banner in the H1, bold opener fragment, badges, one get-started path with the result demonstrated directly below it, an agent block, a visual gap, before/after use cases, feature previews, and a two-branch ending — a lean link-out footer when a docs site exists, the inline tail per `references/standalone-readme.md` when one doesn't. The skeleton itself — per-section rules, the demo-media hierarchy, the agent-block table, and a worked example — lives in `references/readme.md`, the single place it is specified. The minimum viable docs set stays README, LICENSE, CONTRIBUTING, and CHANGELOG.
...
Mode-mixing; a branchy or wall-of-text quickstart; pseudocode or paste-breaking fragments; hand-typed or stale output; bare-ellipsis omissions; retyped instead of embedded examples; untested illustrative-only code; real secrets or domains in samples; hidden credentials in a quickstart; a README that duplicates the docs site; a standalone README that triplicates the command surface across the use cases, get started, and the command table, or enumerates every flag in prose instead of deferring to `--help`; a uvx or pipx tool whose install section narrates the runner ("no install needed — uvx fetches it into a throwaway environment") or pads with a `uv add` alternative instead of just showing `uvx <tool>`; an install section that stacks more than one way to get the same binary (a `go install`, `uv add`, or `pip` alternative under the canonical primary); install prose that narrates the release timeline ("goes live with the first release", "until then, build from a clone", "coming soon"); a quickstart that re-acquires or rebuilds the tool the install section already produced, or runs a binary path the install never created; a README "How it works" section of internals — build mechanics and embed directives, internal event or wire taxonomies, notes on endpoints that don't exist, agent-loop exit conditions; a quickstart example that is an in-joke about the tool's own development, or an inline document duplicating a committed example; delegated prose briefed with a paraphrase of this skill instead of the skill itself; a git-log changelog; future tense or passive that hides the actor; editorial we with no shared technical reality; hedged, buildup-first prose that buries the verdict; fillers like "simply" and "just" in procedure steps; "click here" links; missing alt text; skipped heading levels; generic he or she; biased technical terms; self-referential page or section intros such as "This page explains" or "What to learn"; pre-announced structure such as "three properties follow"; pre-emptive nannying such as "be careful", "make sure", or "don't forget" without real stakes; internal, packaging, or docs-generation detail on task or reference pages or in a README; gratuitous history or legacy references outside explanation pages and the changelog; a cheat sheet shipped as a screenshot or PDF instead of searchable text; a catalog entry whose code is a non-running fragment or whose input and output are hidden; a footgun buried in prose instead of a callout; an analogy stretched past where it holds; a feature list where a before/after use case belongs; full documentation inlined when a docs site exists; a hype-adjective or category-naming opener; hand-composed or stale demo media; a feature preview that documents instead of linking out; and a page that is half cheat sheet and half reference, labeled as one but built like the other.
...
- [ ] The README matches the skeleton in `references/readme.md` — banner in the H1, opener fragment, one get-started path with the result demonstrated directly below, an agent block, and an ending that links out (the docs site, or the tool's own `--help` when standalone) — with no duplicated API content.
- [ ] A standalone README (no docs site) states each command surface once across the use cases, get started, and the command table, carries its reference tables in-file, and defers the exhaustive flag list to `--help` — see `references/standalone-readme.md`.
- [ ] Install prose is timeless — no release-timeline narration or pre-release hedging — and every command after Get started runs the exact artifact the install produced, with every prerequisite named.
- [ ] The README carries no internals: no build mechanics or embed directives, no event or wire taxonomies, no notes on endpoints that don't exist; architecture is at most one paragraph that links out.
