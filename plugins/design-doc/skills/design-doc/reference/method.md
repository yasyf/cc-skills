...
Every design fork goes through a question round. Run it on a live `cc-present` board when that plugin is installed: one card per question with a `choice` block, consequence hints on the options, and a submit bar naming what the round decides — the clicks stream back while you keep working. Fall back to AskUserQuestion when it isn't. The shape is the same on either surface:
