...
| `dd.flow` | `columns: [{id, label, sub?}]` (two to six), `callouts: [{col, title, sub?, status: closed\|open, side?: above\|below, topic?}]` (one to twelve), `legend?: {closed, open}`, `arrows?`, `title?` | the compare panel: the stages of a path as columns, what each one leaks or fixes as callouts |
