...
| `repo` | no | `owner/repo` on GitHub, the repository the doc's links default to: a link into it renders as `#17567`, one anywhere else as `owner/repo#17567`; `check` errors on any other shape. With `repo` set, every `path/file.ext:line` or `:a-b` token in an entry's prose (an assumption's `b`, a decision's `r`, an open item's `t`, a numbers `claim` or `note`) renders as a link to that file and line on GitHub, with a hover card that shows the surrounding lines when the site's token carries Contents: Read |
| `ref` | no | the branch name or commit sha the source links point at; absent, links use `HEAD` and the page rewrites them to the default branch once the GitHub token answers |
...
| `dd.flow` | `columns: [{id, label, sub?}]` (two to six), `callouts: [{col, title, status: fixed\|later, by?, side?: above\|below, topic?}]` (one to twelve), `arrows?`, `title?` | the compare panel: the stages of a path as columns, what leaks or breaks at each one as callouts, each naming in `by` the decision or open item that fixes it |
...
`dd.flow` and `dd.lanes` measure their labels in the page and lay themselves out, once at render and again when the fonts finish loading, so a column is as wide as its widest callout and a lane as tall as its boxes. `dd.flow` stacks a column's callouts outward with a dotted connector, three above before the rest go below, draws `status: later` dashed and `fixed` solid, shows `by` under the title as "Fixed by" or "Left for" followed by that entry's handle (or the phrase as written, or "this design" and "a later design" when `by` is absent), writes its own legend from the two statuses, and, when a callout's `topic` names an `.xs-topic` of the compare panel hosting it, highlights that row on hover. `dd.lanes` frames the two lanes side by side, stacked when the figure is narrower than 640 px, in the `--before` and `--after` tones with "Today" and "Proposed" eyebrows. The `aria-label` is `title`, or a sentence built from the columns or lanes. In the Markdown export a `dd.flow` is one bullet per column with its callouts nested as `title — Fixed by <handle>` and the fixed legend line after; a `dd.lanes` is two bullet lists.
...
| `numbers` | `{id, t, h, claim, source?, cols[], rows[][], note?, component?}` | generic quantitative tables (throughput, capacity, cost, freshness); `id` is `n-<slug>`; `claim` is the sentence rendered above the table saying what it shows and which decision it changes or which risk it sizes, with the decision cited in parentheses; `source` is the grey source line under it; each row has one cell per `cols` entry |
...
Either half stands alone, `{"disabled": true}` publishes a doc with the assistant off, and a file with neither half is an error. `endpoint`, `model`, and `key` come as a set. `reasoning` is `low`, `medium`, `high`, or `none` and sets one effort for every call; absent, chat, the quiz, and read-as run at `high`, the other one-shots at `medium`, and the follow-up suggestions at `low`. A call that ends on `finish_reason: length` with no text is run once more at the next lower effort (`high` to `medium`, `medium` to `low`) before anything is shown; the panel says "Thinking again, with less reasoning…" meanwhile, and a second empty answer reads "The model ran out of room twice; ask a narrower question." `gpt-oss-120b` is the model the assistant is built around; `gemma-4-31b` is the pick only when the assistant has to read an image, and its reasoning is on or off, so `check` warns on a graded value under a `gemma` model. `github.token` is a fine-grained read-only personal access token whose scopes are in [publish.md](publish.md).
