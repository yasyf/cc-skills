...
| Key | Shape | Purpose |
|---|---|---|
| `findings` | `[n, severity, title, ref, link?]` | index of the adversarial review; the prose stays in `<reviewer>-review-<date>.md`. `ref` is the disposition target — usually a `DQ#`, sometimes a spike `V#` or another register's shorthand; `link`, the optional fifth element, is the pull request that fixed the finding, in the shape of a `links[]` entry |
...
`design.py links <dir> [--fetch] [--json] [--missing]` reads the registers and prints one line per decision and open item: the id, the handle, its `s`, and each link as its kind and `owner/repo#n` with a `closes` mark, or `no link` for an entry that has none. `--fetch` asks GitHub for the state of every GitHub link, through `gh api` when the CLI is logged in or through `GITHUB_TOKEN` without it, and prints the state beside the link. It then warns on drift: a `closes` link merged while the item's `s` is still `open`, or `s: "closed"` on an item whose `closes` links are all still open. `--missing` prints only the unlinked entries, the to-do list for the session that shipped the code, and `--json` prints the same rows as JSON for a script. `check` never fetches, so an offline run stays green and the drift report is this command's alone.
