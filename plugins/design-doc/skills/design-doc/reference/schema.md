...
| `repo` | no | `owner/repo`, the GitHub repository most `links` point at; a link chip into it elides the prefix, so `#17567` rather than `Forge-AI/monorepo#17567`. `check` errors on any other shape |
...
| `decisions` | `{id, t, h, s, r, p, key, theme, x?, round?, by?, date?, links?}` | `s` ∈ resolved/superseded/open, rendered Decided / Replaced / Still open; `x` is rejected alternatives; `by` pairs with `s: "superseded"` both ways; `key: true` opens the section as a spotlight card; `theme` is a `themes` key; `links` lists the pull requests that carried the decision into code, rendered as "Where it landed" (see Links below) |
...
| `open` | `{g, id, t, h, p, blocks?, s?, links?}` | `g` must be an `openGroups` key; ids may reference other registers (`DQ15` can sit in the open list); `blocks` lists the decisions this item holds up (`["DQ4"]`), rendered as a chip; `s` ∈ open/closed, default open, the author's word that the item is done; `links` carries the change that retires it, marked `closes: true` (see Links below) |
...
A `links` entry is a bare URL or `{url, kind?, label?, closes?}`. `url` is `https://`. `kind` is `pr`, `issue`, `commit` or `doc`; a `github.com/<owner>/<repo>/pull/<n>`, `/issues/<n>` or `/commit/<sha>` URL infers it, an explicit `kind` on one of those must agree, and `pr`, `issue` and `commit` need a GitHub URL. `label` replaces the chip's text, which defaults to `#<n>` (`<owner>/<repo>#<n>` outside `meta.repo`), the short sha, or the host and path of a `doc`. `closes: true` marks the pull request or issue that retires an open item; it is legal only there, never on a decision, a finding or a commit. Every link is one line in the entry, so a change that carries a decision and retires a spike appears on both, with `closes` only on the spike; `check` warns when one URL sits on two entries.

An open item's `s` is `open` or `closed` and defaults to `open`. It is the author's assertion; a `closes` link that GitHub reports merged or closed is the evidence, and the page treats an item as closed when either holds. `s: "closed"` with no merged link is fine (a sign-off by email closes an item too), which is why `check` never fails on the mismatch and `design.py links --fetch` reports it as drift instead.

The page renders every link as a chip: an icon for the kind, the label, a `closes` tag and, once the site config carries a GitHub token, a dot for the state (open, draft, merged, closed) and a hover card with the title, author, review verdicts and check counts. Each open group's header gains an "N of M closed" bar, a closed row reads "Closed" in its meta, decisions and the reference sheet show "Where it landed", and the Markdown export appends `— #17567 (merged)` to the bullet. The assistant reads the same record through `read_links`.

### Data-only registers (never rendered)

| Key | Shape | Purpose |
|---|---|---|
| `findings` | `[n, severity, title, ref, links?]` | index of the adversarial review; the prose stays in `<reviewer>-review-<date>.md`. `ref` is the disposition target — usually a `DQ#`, sometimes a spike `V#` or another register's shorthand; the optional fifth element lists the pull requests that fixed the finding, in the Links shape below, never with `closes` |
...
`ai.json` sits beside `registers.json` and holds the site config in two halves. The assistant half is what the in-page assistant needs to reach a model, `{"endpoint": "https://api.cerebras.ai/v1", "model": "gpt-oss-120b", "key": "…"}`, or `{"disabled": true}` to publish a doc with the assistant off. The GitHub half is `"github": {"token": "…"}`, a fine-grained read-only personal access token the page uses to resolve the state of every linked pull request and issue. `reasoning` (`low`, `medium`, `high` or `none`) is optional on the assistant half and pins one reasoning effort on every call in place of the page's per-task defaults; `check` rejects a value outside that vocabulary and warns when a `gemma` model carries a graded value, since gemma only switches reasoning on or off. `gpt-oss-120b` is the pick for text; `gemma-4-31b` is the pick only when readers paste images. Either half stands alone; a file with neither is an error, and `disabled: true` beside a `github` block turns off only the assistant. The page reads `../ai.json` first, so a collection configures every doc at once, then `ai.json` beside the doc, and the first file that yields each half wins. With no assistant half the page hides every AI affordance and reads exactly as it did before; with no GitHub half link chips render plain. `localStorage["design-doc-ai"]` overrides the assistant half for local work and `localStorage["design-doc-github"]` (the `{"token": "…"}` block) the GitHub half. The endpoint is an absolute `https://` URL — `http://` only on localhost, since an https page cannot call an http model host — and a config that is not one leaves the assistant off.

The key and the token are readable by anyone who can open the page, so the file is written at deploy time from repository secrets and never committed: `.gitignore` carries it and `ci-check.sh` fails when it is tracked. `meta.ai.suggest` is the only part of the assistant that lives in the registers.

## What `design.py check` enforces

Errors (non-zero exit): required `meta` keys; `meta.draft` a boolean and `meta.draftNote` a non-empty string when present; ID shapes and uniqueness (`A\d+`, `DQ\d+`, `c-[a-z0-9-]+`, `n-[a-z0-9-]+`, `p-[a-z0-9-]+`, integer footnote `n`); status vocabularies; dangling references (`arch.dq`, `arch.a`, `constraints.a`, `findings` refs shaped like `DQ#` or `V#`, `open.g`, `meta.banner.assumption`, `decisions.by`); supersession integrity in both directions (`by` ⇔ `s: "superseded"`); `[^n]` tokens without a footnote entry; malformed `paths.segs`, `ceilings` rows, `scaleMarks`, or `numbers` tables (missing title, bad `cols`, row arity not matching `cols`); when `meta.rev`/`meta.revisions` are present, their shape (positive integer `rev`, non-empty list, integer `rev` and `date` per entry, strictly increasing revs, `meta.rev` equal to the last entry's, `items` and `changed` lists — when present — holding only non-empty strings); `meta.homeLink`, when present, an object with non-empty `href` and `label`; a `summary.html` containing `<html`, `<head`, `<body`, or `<script`, an `<iframe>`, `<object>`, or `<embed>`, an inline `on*=` handler, or an `href`/`src` whose scheme is anything but `http(s)` (checked after entity decoding, so `java&#x0A;script:` is caught); a `diagram` whose `kind` is neither `mermaid` nor `svg`, a `mermaid` one with an empty `source` or no `graph`/`flowchart` header, an `arch[].node` or a fifth `segs[]` element that names no id in the source; a `diagram.overview` that is not a Mermaid flowchart, or that draws a node the full `source` never declares; a `terms[].aliases` that is not a non-empty list of non-empty strings; a `meta.acronyms` that is not a list of non-empty strings, a `meta.ai` that is not an object, or a `meta.ai.suggest` entry that is not a non-empty list of prompt strings; an `h` present but empty; a `theme` that is not a `themes` key; a `tldr` entry that is not `{md, p}`; an `open[].blocks` entry that resolves to no decision; a qa-log `decides` that is not a non-empty list of decision or open-item ids; a `links` entry that is not an https URL or a `{url, kind?, label?, closes?}` object, a `kind` off the list, disagreeing with its GitHub URL, or naming a GitHub kind for a URL that is not one, an empty `label`, a `closes` that is not a boolean or that sits on a decision, a finding or a commit; an `open[].s` outside open/closed; a `meta.repo` that is not `owner/repo`; a `findings` row outside four or five elements; an `ai.json` whose `github` is not `{token}`, or that carries neither the assistant keys nor a `github` block.

Warnings (advisory): footnotes never referenced; registers `rounds` entries no decision points at, or missing from qa-log; a decision `round` with no registers `rounds` entry; p95 below p50; qa-log answers that match no offered label (legal — confirm they were intended); a qa-log question lacking both `decides` and an id in its header or text, in a round that settled two or more decisions (the replay cannot tell which one it produced); revision numbers not contiguous from 1; a listed revision whose `history/rev-<N>.json` is missing or unparsable (the changes-since picker can't diff against it); a revision note past ~90 characters (keep the note a headline; detail goes in `--item` bullets); no `summary.html`, or one still carrying the scaffold's TODO (either way the doc opens without an executive summary); a register ID cited in `summary.html` (`DQ#`, `A#`, `Q#`, `V#`, `c-<slug>`) that no register defines; a `sysd.svg` with no `viewBox`, and a `diagram` of kind `svg` at all ("hand-drawn diagram"); a `LIBS` pin in the template that differs from the versions stated above; a qa-log `question`, `topic`, or `header`, or a registers `rounds[].q`, that starts with a register ID or contains "finding N"; a decision `t` that ends in a question mark or runs past twelve words; a handle outside two to five words; a term defined but never named outside its own definition, aliases counted; a `meta.ai.suggest` key that is not a section id; a link URL that appears on two entries; an `ai.json` key the page ignores; and every capitalisation offence below. When the entry at `meta.rev` carries `files`, `check` rehashes `summary.html` (and `sysd.svg`, when used) against those digests and, on a mismatch, warns that the file changed since that revision; it compares no earlier entry.
