...
`ai.json` sits beside `registers.json` and holds what the in-page assistant needs to reach a model: `{"endpoint": "https://api.cerebras.ai/v1", "model": "gpt-oss-120b", "key": "…"}`, or `{"disabled": true}` to publish a doc with the assistant off. Two keys are optional: `reasoning` (`low`, `medium`, `high` or `none`) pins one reasoning effort on every call in place of the page's per-task defaults, and `github` (`{"token": "…"}`) is the read-only token the page uses to resolve pull-request links; `check` rejects a `reasoning` outside that vocabulary and warns when a `gemma` model carries a graded value, since gemma only switches reasoning on or off. `gpt-oss-120b` is the pick for text; `gemma-4-31b` is the pick only when readers paste images. The page reads `../ai.json` first, so a collection configures every doc at once, then `ai.json` beside the doc. With neither, the page hides every AI affordance and reads exactly as it did before; `localStorage["design-doc-ai"]` overrides both for local work. The endpoint is an absolute `https://` URL — `http://` only on localhost, since an https page cannot call an http model host — and a config that is not one leaves the assistant off.
