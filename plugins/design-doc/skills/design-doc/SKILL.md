...
- **Component** — an interactive figure declared in the `components` map (`dd.tabs`, `dd.before-after`, `dd.whatif`, `dd.steps`, `dd.timeline`, `dd.matrix`, and the two compare-panel figures `dd.flow` and `dd.lanes`) and placed by id in the deck or on an entry; a TSX file under `components/` is the escape hatch `build` compiles.
- **Figure brief** and **critique** — the two NOTES.md sections the deck is drawn from and read against. A brief is five lines per figure, written before the figure is drawn: the point it makes, the decision it serves by ID, the smallest view that makes the point, what it leaves out, and the real labels it uses. Its first two lines become the `<figcaption>`. The critique is one row per panel, figure, table, and stat, written before the gates: the decision it serves, the claim it makes, the question a reader is left with, and fixed or cut. `check` warns when either heading is missing.
- **Link** — an entry of `links[]` on a decision, an open item, or a finding row: the pull request, issue, commit, or doc that lands it. `closes: true` on an open item's link marks the one that retires it. The doc shows each link as a chip with its GitHub state and heads every open group with how many of its items are closed.
- Statuses: decisions are `resolved | superseded | open`, which the doc renders as Decided, Replaced, and Still open; assumptions are `working | validate`, rendered Assumed and Needs someone to confirm (someone outside the document has to say yes). Open items are `open | closed`, rendered Still open and Closed; a merged `closes` link closes an item before `s` says so. Prose uses the rendered words, JSON the values.
...
Read [reference/writing.md](reference/writing.md) first; the deck contract, the twin contract, and the voice contract live there, and its Figures section sends you to `reference/gallery/` before you draft. Fill `meta` in registers.json (title, date, slug, banner for the starred assumption). Write `summary.html` before anything else: a deck of full-screen panels, thesis first, built from the diagnosis (what exists today, or what is required), the resolved decisions (what this proposes), and the numbers, with one figure drawn for this design on every panel. When the design is one idea with one figure, write a single poster panel instead and say so in NOTES.md. The panel kinds, the kit, and the word budgets are in [reference/schema.md](reference/schema.md); `check` enforces the budgets. Put the system diagram's Mermaid source in the `diagram` register key, its nodes named after the `arch` cards they belong to, so the doc can pan it, highlight it, and step the request paths through it. Write `diagram.overview` beside it: the same graph cut to ten nodes, ids drawn from the full source, which is what the Overview card draws. Where a number or a sequence earns it, declare one or two components in the `components` map and place them, in a deck figure or on an entry; the kinds and the escape hatch are in [reference/writing.md](reference/writing.md). The compare panel's figure is a kit component too: `dd.flow` for stages in columns with what each one leaks or fixes hanging off them, `dd.lanes` for today beside proposed as two stacks of boxes. Both measure their labels and lay themselves out, so nothing overlaps at any width; a hand-drawn SVG is for a shape the kit lacks, and `check` says so when it meets one with eight or more boxes. Every label, in the diagrams, the deck, the chips, and the table heads, follows one capitalisation rule: acronyms upper-case (Tenant API, DPoP), product names as their owners write them (Postgres), identifiers in backticks, everything else sentence case. `check` lints labels against the acronym list in `design.py`, extended by `meta.acronyms`. Then set `key: true` on the 3–8 decisions and assumptions the doc should open on and file every one under a theme from Phase 0; write the tl;dr as three to five plain twins of at most 20 words each. Both files sit beside `registers.json` and the doc loads them at runtime, so the HTML itself is never edited. Then write the doc content in two passes: structure and de-jargoning first, then a separate tone pass whose test for every sentence is "does this solicit feedback, or make a claim?"; the doc exists to be corrected, not admired.

The deck is one argument, and every part of it names the decision it serves; the contract is in [reference/writing.md](reference/writing.md) under One argument. No figure is drawn before its brief, the thesis diagram included: five lines under "Figure briefs" in NOTES.md, whose first two lines become the figure's `<figcaption>`, and a figure whose brief cannot name a decision is not drawn. The system diagram draws the proposed system by role: one node per role, the one thing the design decides about it on a second line, and every edge labelled by what flows on it. Today's state lives in the compare figure, beside the proposal, and nowhere else. Every panel headline is a claim with a verb in it, every figure and table carries a caption citing its decision, every `numbers` table opens on its `claim`, and every stat's label cites the decision it changes or the risk it sizes.

Before the gates, run the critique pass: one row per panel, figure, table, and stat under "The critique" in NOTES.md, with the decision it serves, the claim it makes, the question a reader is left with, and whether it was fixed or cut. A row with no decision is cut, not argued for. Then read the headlines alone; they have to carry the argument on their own.
...
**Exit criteria:** the doc renders over `python3 -m http.server 8641` and opens on the deck with nothing else visible; the PDF is built and visually inspected. Every figure has a brief and a caption naming its decision; every `numbers` table opens on its claim; the critique has a row for every panel, figure, table, and stat, and none lacks a decision. All five gates ran, in order, and every finding they raised is triaged in NOTES.md.
...
A doc with a `components/` directory runs `$TOOL build .` first and copies `components.js` too. The site config is an `ai.json` beside `index.html`, or one directory up for a collection, and it switches on two things: the assistant, from `{endpoint, model, key, reasoning?}`, and the GitHub state on every link chip, from `github: {token}`. Either half stands alone, `{"disabled": true}` turns the assistant off, and both halves are written at deploy from secrets and never committed; the token's scopes are in [reference/publish.md](reference/publish.md).
...
The register keeps recording once the proposal is accepted. Every pull request or issue that carries a decision or retires an open item goes on that entry's `links[]` the moment it opens, with `closes: true` on the open item it retires. A finding fixed by code takes the same link as its row's fifth element. When the pull request merges, set `s: "closed"` on the item, run `$TOOL links <dir> --fetch` so the states GitHub reports are checked against the register, and snapshot, so a returning reader sees what landed. `$TOOL links <dir> --missing` prints the entries still unlinked; it is the to-do list for a session that shipped the code and skipped the link. One link per entry, on the pull request that lands it, and the handle and twin stay as they were: the contract is in [reference/writing.md](reference/writing.md), the field shapes in [reference/schema.md](reference/schema.md).

**Exit criteria:** the user has the URL or serve command; the changelog records what shipped; the revision snapshot is recorded, its note and items having passed the voice gate first; `$TOOL links <dir> --missing` lists nothing the user did not knowingly leave unlinked.
...
- **No Ask bar at the bottom right, no Cmd-K AI row** — the page found no `ai.json` beside `index.html` or one directory up, or it holds `{"disabled": true}`. Local work sets the `design-doc-ai` key in `localStorage`; a deploy writes the file from a secret, per `reference/publish.md`.
- **Link chips show no state dot, no hover card, no progress bar** — the site config carries no `github.token`, or the token cannot see the repository, which renders as unknown. Local work sets the `design-doc-github` key in `localStorage` to `{"token": "…"}`; a deploy writes that half from its own secret. `$TOOL links <dir> --fetch` reads the same states from the command line.
