...
- **No Ask button, no Cmd-K AI row** — the page found no `ai.json` beside `index.html` or one directory up, or it holds `{"disabled": true}`. Local work sets the `design-doc-ai` key in `localStorage`; a deploy writes the file from a secret, per `reference/publish.md`.
