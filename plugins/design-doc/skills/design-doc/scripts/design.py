...
not. Stdlib only.
"""
import argparse, copy, datetime, importlib.util, itertools, json, math, os, re, shutil, subprocess, sys, tempfile, urllib.parse, urllib.request
...
def pdf(args) -> int:
    return load_builder().build(Path(args.dir), "design-doc.pdf")
...
def render_check(args) -> int:
    ...
    if not page:
        print(f"render-check: {root} holds none of {', '.join(builder.DOC_PAGES)}.", file=sys.stderr)
        return 1
...
def check_template_fresh(rep):
    ...
    if not shared.exists():
        return
    spec = importlib.util.spec_from_file_location("shared_build", shared / "build.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    problem = module.stale_host(TEMPLATES / "design-doc.html")
    if problem:
        rep.err(problem)
