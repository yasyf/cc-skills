...
message into `desk.py` as it arrives, so the shapes below are also the tool's inputs.
...
The desk records it as `desk.py report --pr <n> --head <sha> --lane <name> --verdict
<v> --text "<line 2>"`. The same PR, head, and verdict twice is one report: the second
is dropped and never answered. A new head is a new report.
...
`desk.py ruling --text "<question>" --options "A <..>|B <..>|C <..>"` records it and
...
`desk.py summary` prints it; the desk sends it unchanged. Line one is always the
...
git log by `desk.py landed`. It never reads a PR's `merged` field, which the Graphite
...
first as `desk.py enqueue --kind idle` and the tool drops the rest as duplicates. No
...
once per head and failing job, printed by `desk.py route`:
...
A second red on the same head is not routed again. A new head that is still red is.
