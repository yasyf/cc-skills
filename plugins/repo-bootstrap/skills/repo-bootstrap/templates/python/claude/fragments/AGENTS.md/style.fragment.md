...
**Mechanical linting.** Running `ruff` is fine, and encouraged — the pre-commit hooks (prek: ruff + ty) also auto-format, fix import order, and print whole-project type warnings on every `git commit` (ty never blocks a commit); run `uvx prek install` once to activate them, or `uvx prek run --all-files` to clean everything up-front. Fix what needs human judgment and let the tooling own the mechanical churn. When reviewing code, don't flag mechanical lint violations (line length, whitespace, import order, trailing commas).
