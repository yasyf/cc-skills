...
- **Rendered artifact** — `AGENTS.md`, `CLAUDE.md`, and the plugin installer are generated: scaffold writes an `X.src.<ext>` source (repo prose + bare `{{> name}}` cc-guides directives), then the post-write step runs `cc-guides render`, which expands the directives — whose fragment bodies live in the `cc-guides` binary — into the banner-stamped `X.<ext>`. cc-guides is a hard prerequisite (`brew install yasyf/tap/cc-guides`); there is no Python fallback. See `reference/base-conventions.md` and the [cc-guides](https://github.com/yasyf/cc-guides) README.
...
- **`cc-guides` is a prerequisite.** After writing files, scaffold runs `cc-guides
  render` in the target to turn each `X.src.<ext>` (AGENTS/CLAUDE/the plugin
  installer) into its banner-stamped sibling; a missing binary is a hard error
  (`brew install yasyf/tap/cc-guides`). A rendered `X.<ext>` is generated — edit
  `X.src.<ext>` and re-run `cc-guides render`.
...
|---|---|---|
| `AGENTS.src.md` → `AGENTS.md`, `STYLEGUIDE.md`, `README.md` | base; python/go/swift/swift-app **override** | `AGENTS.md` is a rendered artifact (scaffold writes `AGENTS.src.md` with bare `{{> name}}` directives; `cc-guides render` expands them); every README follows the writing-docs skeleton (`references/readme.md`); the language versions gate the get-started path, agent-block invocation, and docs teaser on `--features` (uvx/PyPI + docs for python; brew `release` for go and swift); swift-app shares the swift `STYLEGUIDE.md` |
| `CLAUDE.src.md` → `CLAUDE.md`, `CHANGELOG.md`, `LICENSE`, `.gitignore` | base | `CLAUDE.md` is a rendered artifact too (`CLAUDE.src.md`, body `@AGENTS.md`) plus Claude-only rules (AskUserQuestion, task tracking, plan execution & orchestration); `.gitignore` gains python/go/swift entries when layered; `LICENSE` omitted with license `none` |
...
| `plugin/scripts/install-binary.src.sh` → `install-binary.sh` | extra `plugin` | the **canonical plugin binary installer**, a rendered artifact — `bin/<name>` only ever a symlink (brew binary first, sha256-verified download into `CLAUDE_PLUGIN_DATA` as fallback, dev builds never clobbered). Scaffold writes a one-line source (`{{> install-binary-pinned\|latest binary=… repo=… brew=… plugin=…}}`); `cc-guides render` expands it into the real installer, whose body lives in the cc-guides binary. See `reference/go-ci-and-release.md` § `format: binary` archive |
...
post-write `cc-guides render` produced `AGENTS.md`/`CLAUDE.md` beside their `.src`);
